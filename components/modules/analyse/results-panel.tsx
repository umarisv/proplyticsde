...
export function ResultsPanel({ data, formData, onRecalculate, isCalculating }: ResultsPanelProps) {
  ...
  return (
    <div className="flex flex-col gap-4 p-4 h-full overflow-y-auto custom-scrollbar">
      ...
      {/* ── Marktwert ──────────────────────────────────────────────────── */}
      <div className="rounded-xl border-2 border-primary bg-primary p-5 text-center shadow-md">
        <p className="mb-1 text-xs font-medium uppercase tracking-wider text-primary-foreground/80">Geschaetzter Marktwert</p>
        <p className="text-3xl font-bold tracking-tight text-primary-foreground sm:text-4xl">{formatCurrency(data.marktwert)}</p>
        <div className="mx-auto mt-3 flex max-w-xs items-center justify-between rounded-lg bg-primary-foreground/15 px-4 py-2 text-xs">
          <div className="text-center">
            <p className="text-primary-foreground/70">Min</p>
            <p className="font-semibold text-primary-foreground">{formatCurrency(data.marktwertMin)}</p>
          </div>
          <div className="h-6 w-px bg-primary-foreground/20" />
          <div className="text-center">
            <p className="text-primary-foreground/70">Marktwert</p>
            <p className="font-semibold text-primary-foreground">{formatCurrency(data.marktwert)}</p>
          </div>
          <div className="h-6 w-px bg-primary-foreground/20" />
          <div className="text-center">
            <p className="text-primary-foreground/70">Max</p>
            <p className="font-semibold text-primary-foreground">{formatCurrency(data.marktwertMax)}</p>
          </div>
        </div>
      </div>
