...
export async function createPost(formData: FormData) {
  ...
  if (postType === "meinungsbild") {
    objektdaten = { adresse: (formData.get("obj_adresse") as string) || "", objekttyp: (formData.get("obj_objekttyp") as string) || "", baujahr: (formData.get("obj_baujahr") as string) || "", wohneinheiten: (formData.get("obj_wohneinheiten") as string) || "", kaufpreis: (formData.get("obj_kaufpreis") as string) || "", mieteinnahmen: (formData.get("obj_mieteinnahmen") as string) || "" }
  }
...
export async function aggregateRatings(ratings: MeinungsbildRating[]): Promise<MeinungsbildAggregation> {
  if (ratings.length === 0) return { count: 0, rendite: 0, risiko: 0, finanzierung: 0, value_add: 0, lage_markt: 0, deal_sourcing: 0 }
  const dims = ["rendite", "risiko", "finanzierung", "value_add", "lage_markt", "deal_sourcing"] as const
  const agg: Record<string, number> = {}
  for (const d of dims) { agg[d] = Math.round((ratings.reduce((s, r) => s + r[d], 0) / ratings.length) * 10) / 10 }
